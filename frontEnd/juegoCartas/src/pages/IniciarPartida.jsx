...
const IniciarPartida = () => {
  ...
  const [modall, setModal] = useState({
    isOpen: false,
 ...
 /* useEffect(() => {
  ...
  useEffect(() => {
    const compararCodigo = async () => {
      try {
        const response = await axios.post(
          "http://localhost:8089/partida/comparar",
          {
            codigo,
          }
        );
  ...
  const actualizarPartida = async () => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:8089/partida/${idpartida}`,
        {
          participantes: user,
        }
      );
      ...
      <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-b from-gray-100 to-gray-300">
            ...
            <div className="flex flex-col justify-center items-center w-96 bg-white p-6 rounded-lg shadow-lg">
              {alert && (
                <h1 className="text-lg mb-4">
                  El código proporcionado no pertenece a ninguna partida
                </h1>
              )}
              <h2 className="text-center font-semibold text-2xl text-gray-700 mb-6">
                Escriba su código para iniciar partida
              </h2>
        ...
        <div className="w-5/6 max-w-5xl bg-white shadow-lg rounded-3xl p-8 flex flex-col items-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {datosPartida &&
              datosPartida.map((partida, index) => {
                return (
                  <PartidasDisponibles
                    key={partida._id}
          ...
          <div className=" w-full flex justify-end items-end">
            <button className="relative cursor-pointer bg-gradient-to-r from-blue-600 to-indigo-500 p-6 rounded-xl border-white border-2 w-40 shadow-lg transform transition-transform hover:scale-105 text-white">
              Partidas a las que perteneces
            </button>
          </div>
        </div>
      </div>
